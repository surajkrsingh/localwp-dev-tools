...
function registerCronHandlers() {
	...
	ipcMain.on('debugging-tool:get-cron-events', async (event, siteId: string) => {
		try {
			const site = requireSite(siteId);
			const raw = await runWpCli(site, ['cron', 'event', 'list', '--format=json']);
			event.reply('debugging-tool:cron-events-result', { events: JSON.parse(raw) });
		} catch (error: any) {
			replyError(event, 'debugging-tool:cron-events-result', error);
		}
	});

	/** Run a cron event immediately. */
	ipcMain.on('debugging-tool:run-cron-event', async (event, siteId: string, hook: string) => {
		try {
			const site = requireSite(siteId);
			await runWpCli(site, ['cron', 'event', 'run', hook]);
			// Return refreshed list
			const raw = await runWpCli(site, ['cron', 'event', 'list', '--format=json']);
			event.reply('debugging-tool:run-cron-result', { events: JSON.parse(raw) });
		} catch (error: any) {
			replyError(event, 'debugging-tool:run-cron-result', error);
		}
	});

	/** Delete a scheduled cron event. */
	ipcMain.on('debugging-tool:delete-cron-event', async (event, siteId: string, hook: string) => {
		try {
			const site = requireSite(siteId);
			await runWpCli(site, ['cron', 'event', 'delete', hook]);
			const raw = await runWpCli(site, ['cron', 'event', 'list', '--format=json']);
			event.reply('debugging-tool:delete-cron-result', { events: JSON.parse(raw) });
		} catch (error: any) {
			replyError(event, 'debugging-tool:delete-cron-result', error);
		}
	});
}
...
function registerUserSwitcherHandlers() {
	/** List all WordPress users. */
	ipcMain.on('debugging-tool:get-users', async (event, siteId: string) => {
		try {
			const site = requireSite(siteId);
			const raw = await runWpCli(site, ['user', 'list', '--format=json']);
			event.reply('debugging-tool:users-result', { users: JSON.parse(raw) });
		} catch (error: any) {
			replyError(event, 'debugging-tool:users-result', error);
		}
	});

	/** Generate a one-time login URL for a user and open it in the browser. */
	ipcMain.on('debugging-tool:login-as-user', async (event, siteId: string, userId: number) => {
		try {
			const site = requireSite(siteId);

			// Generate a one-time login nonce via WP-CLI eval
			const phpCode = `
				$user = get_user_by('id', ${userId});
				if (!$user) { echo 'ERROR:User not found'; exit; }
				$key = wp_generate_password(20, false);
				set_transient('devtools_login_' . $key, $user->ID, 60);
				echo home_url('?devtools_autologin=' . $key);
			`.replace(/\n\t+/g, ' ').trim();

			const loginUrl = await runWpCli(site, ['eval', phpCode]);

			if (loginUrl.startsWith('ERROR:')) {
				event.reply('debugging-tool:login-as-result', { error: loginUrl.replace('ERROR:', '') });
				return;
			}

			// Install the auto-login handler as a mu-plugin (one-time setup)
			await ensureAutoLoginMuPlugin(site);

			// Open the URL in the default browser
			electron.shell.openExternal(loginUrl.trim());
			event.reply('debugging-tool:login-as-result', { success: true, url: loginUrl.trim() });
		} catch (error: any) {
			replyError(event, 'debugging-tool:login-as-result', error);
		}
	});
}

/** Ensure the auto-login mu-plugin exists for the site. */
function ensureAutoLoginMuPlugin(site: any): Promise<void> {
	const sitePath = getSitePath(site);
	const muPluginsDir = path.join(sitePath, 'app', 'public', 'wp-content', 'mu-plugins');
	const muPluginPath = path.join(muPluginsDir, 'devtools-autologin.php');

	if (fs.existsSync(muPluginPath)) return Promise.resolve();
...
/**
 * LocalWP Dev Tools — Auto Login Handler
 *
 * Handles one-time login links generated by the User Switcher tool.
 * The login key is a transient that expires after 60 seconds.
 */
add_action('init', function() {
    if (empty($_GET['devtools_autologin'])) return;

    $key = sanitize_text_field($_GET['devtools_autologin']);
    $user_id = get_transient('devtools_login_' . $key);

    if (!$user_id) {
        wp_die('Login link expired or invalid.', 'Login Failed', ['response' => 403]);
    }

    delete_transient('devtools_login_' . $key);
...
`;
	...
	fs.writeFileSync(muPluginPath, phpContent, 'utf8');
